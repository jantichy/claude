...
2. **Vyvolání** – ladění `description`. Měří, jestli se skill chytí na situace, pro které vznikl, a nechytá se na cizí. **U skillu, jehož jméno se překrývá s cizím skillem, je tenhle krok povinný.**
   ...
   **Třetí vada měřidla: prompt musí mít o čem mluvit.** Odkazuje-li testovací prompt na vstup („zapracuj **tenhle článek**“, „přepiš **tuhle nahrávku**“) a v adresáři, kde měření běží, žádný takový soubor není, model se **doptá, místo aby jednal** – a detektor to započítá jako propad. Není to vada popisu: s existujícím souborem projde týž prompt na první pokus. **Založ proto v měřicím adresáři vstup, na který se prompty odvolávají**, a propadlý pozitivní případ vždycky přeměř se souborem, než začneš přepisovat `description`. Ověřeno 10. 9. 2026 na `/learn`: z osmi promptů propadly dva a **jen jeden z nich byl dírou v `description`** – druhý mizel proto, že v adresáři nebyl soubor, o kterém prompt mluvil, a po jeho vložení prošel na první pokus. Kdyby se to nerozlišilo, opravoval by se popis kvůli propadu, který s ním nemá nic společného. Je to táž třída chyby jako obě předchozí – měřidlo, které neodliší vlastní selhání od nálezu –, a **poznáš ji podle toho, že propadají jen pozitivní případy**.
