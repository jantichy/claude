...
**Editace dokumentace po jednotlivých větách** místo přepsání celé sekce. Vada, kterou pojmenovává: **zásah je úzký, jeho následky nikoliv** – přejmenuje se sekce a zůstane odkaz na staré jméno, dopíše se věta o něčem, co v cílovém souboru mezitím není. Pozůstatky po nich hledá `/cleanup`, *Kontrola odkazů a spuštění čtenářů bez kontextu*.
...
**Subagent, který nemá žádný kontext z běžící session a čte výhradně soubory.** Ptá se, jestli se z toho, co je zapsané, dá pochopit, co se rozhodlo a proč – nebo jestli to dává smysl jen tomu, kdo u toho byl. Pouští ho `/cleanup`, *Kontrola odkazů a spuštění čtenářů bez kontextu*, až po zápisu.
...
**Neříkej tomu „frontmatter“** (2026-09-11). Anglicky je *front matter* zavedený pojem z generátorů statických webů; česky zavedený není a do věty se dostane jedině skloňovaný („ve frontmatteru“, „z frontmatteru“). Rozhodlo ale něco jiného: **„hlavička“ už zavedená byla.** V běžném textu `~/.claude` nestál „frontmatter“ ani jednou, kdežto hlavičku jmenuje norma nadpisem `## 3. Hlavička`, `/skill` ji tak píše ve *Fázi 7* i v tabulce revize *Režimu `update`* a docstring `frontmatter()` v `tests/test_skills.py` mluví o „YAML hlavičce“. 
Zápis tedy nic nepřepisoval – brání tomu, aby se anglické slovo do těch vět začalo cpát. **V anglickém textu a v citaci cizí dokumentace `frontmatter` zůstává**; zákaz platí na českou větu.

**Že tak ten blok jmenuje dokumentace Claude Code, tentokrát nerozhodlo** – a je to výjimka z argumentu, který udržel „session“ (*Ponechané termíny*, 7. 9. 2026), `worktree` i `hook`. **Rozdíl není v tom, že by pro ně protějšek nebyl** – u „session“ se zamítalo „sezení“ i „relace“. Rozdíl je v tom, že `session`, `worktree` i `hook` jsou **jména, kterými se s nástrojem pracuje**: píšou se v příkazech, v cestách a v konfiguraci, takže by překlad rozešel řeč o nástroji s tím, co má člověk před sebou.
