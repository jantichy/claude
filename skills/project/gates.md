# Kontrakt příkazů, zelená linka a brány kvality

Podrobnosti ke kroku 12 v `SKILL.md`: šablona sekce `## Příkazy`, co se zapnutím vzniká, jak se dává souhlas a které brány se nastavují konfigurací.
...
Definice a prahy jednotlivých bran jsou v `~/Dev/context/coding/quality.md`. Řekni uživateli jednou větou, co se právě zapnulo – ne aby ho to překvapilo, až mu hook poprvé zablokuje konec tahu.

**Zapni i brány, které se nespouštějí příkazem, ale konfigurací.** Kontrakt říká, *čím* se kontroluje; tyhle určují, *jak přísně*. Bez nich zůstanou prahy z `quality.md` jen napsané a nikdo je neměří:
