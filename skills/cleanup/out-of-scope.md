...
5. **Projdi je jednu po druhé.** U každé ji nejdřív vypiš:
   ...
   **Nejdřív ale rozhodni, do které fronty položka věcně patří.** *Zapsat do todo* i *Zapsat do backlogu* míří na soubory **uklízeného projektu**. Položka, která patří jinam – obecné pravidlo do `~/.claude/RULES.md`, poznámka ke skillu nebo úkol na konfigurační vrstvě do `~/.claude/todo.md`, doménová znalost do `~/Dev/context/` –, do nich nepatří: obecné pravidlo zapsané do `docs/todo.md` jednoho projektu se schová tam, kde ho nikdo nehledá. U takové položky **skládej volby věcně** – kam a v jaké podobě ten zápis patří –, ne z předepsané čtveřice. K tobě se dostane vždycky, protože agent mimo projekt zapisovat nesmí ([`agent.md`](agent.md), *Co nesmíš*). Doloženo 25. 9. 2026.
