---
name: attack
description: Skill se použije, když uživatel zadá "/attack", "/attack branch" nebo "/attack full", anebo chce hotovou práci prověřit tak, že se ji někdo pokusí reálně rozbít – spustit aplikaci, sahat na ni mimo šťastnou cestu, posílat nesmyslné vstupy, lámat stavy a hledat, co spadne. Na rozdíl od /review, který kód čte, tenhle skill ho spouští. Běží výhradně proti lokální instanci, nikdy proti produkci.
...
- **`/attack`** nebo **`/attack branch`** (výchozí) – útočí se na to, čeho se dotkla práce na aktuální větvi: obrazovky, endpointy a toky, které se změnily nebo na změněný kód navazují.
