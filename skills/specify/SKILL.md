...
0. **Nejdřív si přečti, co v projektu už je** – zejména `docs/demand.md`, `docs/competition.md` a `docs/risks.md` od `/discovery`, `docs/operation.md` od `/evaluate`, a `docs/backlog.md` a `docs/todo.md` (viz bod 5). **`operation.md` je u přírůstku do běžícího produktu ten nejcennější vstup**, protože jako jediný nese doklady o tom, co lidé s hotovou věcí doopravdy dělají – co nedokončili, co si vyžádali, co jim systém odmítl. Poznatek v něm je **vstup, ne rozhodnutí**: co se z něj postaví, rozhoduje tenhle krok, a zpátky se do něj nepropisuje nic. `demand.md` odpovídá na to, proč to stavíme a čím je to doložené; sekce *Co poměřujeme* na to, jaký problém řešíme a komu; *Naše pozice a odlišení* říká, co produkt musí umět a čím se liší; rizika říkají, co musí být postavené jinak. **Na nic z toho se neptej znovu** – shrň to a nech potvrdit.
