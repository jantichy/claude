...
- Kód piš **anglicky** – **každý identifikátor**: proměnné, konstanty, funkce, třídy, soubory a adresáře, testy (jméno souboru, třídy i testovací metody), klíče v datech a ve schématech výstupu agentů, přepínače, proměnné prostředí a zástupné symboly v příkazech, cestách a jménech souborů (`<project>`, ne `<projekt>`). **Argumenty slash příkazu** v `argument-hint` a v ukázce volání skillu (`/invoicing recover <klient>`) jsou výjimka a píšou se česky: píše je uživatel v rozhraní a čte je v nápovědě jako popis, ne jako jméno, které by stroj porovnával. Zástupný symbol v příkazu shellu nebo v cestě doplňuje Claude do kódu, a ten je anglicky. **Režimy skillu výjimkou nejsou** – jsou to jména a píšou se anglicky (`~/.claude/skills/SKILLS.md`, *Hlavička*).
- **Hodnoty v datech se řídí tím, kdo je čte.** Hodnota, kterou vyrábí kód a jiný kód podle ní rozhoduje (`merge_pending`, `occupied`, značka v logu), je identifikátor – anglicky. Hodnota ze slovníku, který se vypisuje člověku doslova (škála závažnosti `KRITICKÉ`, míra jistoty `vysoká`), zůstává česky. Pole smí nést obojí jen tehdy, když česká hodnota přichází zvenčí – z textu, který píše uživatel (řádek *Stav* v `todo.md`).
