...
- **Tenhle repozitář má `todo.md`, `backlog.md`, `done.md` i `decisions.md` v `~/Dev/context/`**, ne u sebe. Platí to pro každý skill, který do nich zapisuje a zároveň se tu pouští – `/review`, `/oponent`, `/consistency`, `/cleanup`, `/implement` a `/skill`. (`/attack` a `/release` se tu nepouštějí vůbec, viz výš.) **U `/skill` to platí bezvýhradně**, protože jako jediný nikde jinde běžet nemůže – spravuje skilly, a ty jsou jen tady. A **neptej se na to pokaždé znovu**:
  ...
  | záznam dokončeného průchodu (`## Průchody životním cyklem`) | `~/Dev/context/done.md` | **vždy**, i u `/review`. Původně se u něj zapisovalo jen tehdy, „když má smysl ho pak číst“, protože jeho čtenářem je `/release` – a ten se tu nepouští. Jenže čtenáři jsou dva další: příští `/review`, který jinak nepozná rozsah předchozího běhu, a člověk, který z repozitáře nezjistí, že běh vůbec proběhl. Odhalil to čtenář bez kontextu 8. 9. 2026: po `/review full` s 33 nálezy nezůstala v repozitáři jediná stopa krom tvaru commitů. **U `/oponent`** je čtenářem příští `/oponent`, který podle svého SKILL.md bez seznamu hledisek neví, s čím se má srovnávat |
