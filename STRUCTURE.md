...
- **`/consistency` a `/cleanup`** – čte je příští běh téhož skillu: `/consistency` pozná, co už bylo auditované a s jakým vědomě zúženým rozsahem, `/cleanup` co zůstalo mimo rozsah úklidu a jak se s tím naložilo. **`/cleanup` k tomu od 25. 9. 2026 uvádí i id uklizené session** – bez něj z řádku nejde poznat, co se vlastně uklidilo, a je to tím podstatnější, že se skill chystá přepsat na běh v subagentovi nad cizí session. Není to evidence ani čára: opakovaný běh nad toutéž session je legitimní a dvě data u téhož id znamenají dva úklidy.
