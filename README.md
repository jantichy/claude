...
## Standardní struktura projektu – každý projekt vypadá stejně

Zjistil jsem, že v každém projektu chci ty samé čtyři věci vedle `CLAUDE.md`: `README.md` pro člověka a v `docs/` trojici `todo.md` (co jsem odložil), `decisions.md` (co jsem rozhodl a **proč**, včetně variant, které jsem zavrhl) a `rules.md` (principy, ve kterých se ten konkrétní projekt pohybuje). Nejcennější je na tom to poslední: jak projekt roste, vybrušují se hranice, ve kterých se rozhodujeme – a zapsané mi šetří to, abych je vymýšlel pokaždé znovu. Konvenci definuje na jednom místě soubor `structure.md` – ten žije v mé soukromé knowledge-base (`~/Dev/context/`), takže v tomhle repu ho nenajdete. `/project` ji instaluje a `/cleanup` kontroluje, že ji Claude opravdu průběžně plní.
...
Zapne pro projekt automatické logování všech mých promptů na jedno snadno přístupné a čitelné místo, do souboru `docs/prompts.md` v projektu. Při zapnutí dokonce backfilluje historii ze session souborů, takže získám kompletní záznam i zpětně. Ač to na první pohled nevypadá, dříve či později to opravdu doceníte. Pro sledování, jak se projekt vyvíjel, pro učení se vlastních anti-patternů v promptech, překvapivě často tam pošlete Clauda, aby si z toho něco zpětně vytahal nebo naučil…

## [`/project`](skills/project/) – projekt nastavený na pár kliknutí

Kdykoli zakládám projekt, řeším pořád dokola ty samé věci: git a remote, autocommit, autoprompt, jestli si Claude smí něco pamatovat do Memory nebo má všechno psát do `CLAUDE.md`, jaký je to vlastně typ projektu. Tenhle skill se mě na to všechno postupně zeptá – jedna otázka po druhé – a rovnou to nastaví, včetně `README.md`, `.gitignore` a standardní struktury podle `structure.md`. Umí i **projekty, které už existují**: udělá inventuru toho, co v adresáři je, dorovná to na moje dnešní preference a nikdy nepřepíše soubor bez zeptání. Zeptá se taky, jestli má být projekt rozbalený klasicky, nebo ve **worktree layoutu** – kontejner s `.bare` a jedním podadresářem na větev, díky kterému nad projektem můžu jet ve třech Claude sessions naráz, aniž si přepisují soubory. A podle charakteru projektu do jeho `CLAUDE.md` natvrdo naimportuje jen ty doménové checklisty, které dávají smysl.
...
- **Část znalostí v repu není.** `/compose` čte `~/Dev/context/compose/` a `~/Dev/archiv/`, doménové checklisty a definice struktury projektu žijí v `~/Dev/context/` jako `web.md`, `admin.md`, `coding.md`, `text.md`, `worktrees.md` a `structure.md` – odkazuje na ně `CLAUDE.md` a většinu z nich používá skill `/standards`, který bez nich neudělá nic. To je moje soukromé know-how a osobní archiv, takže je tu nenajdete – ty skilly jsou k mání jako kostra, ne jako hotová věc.
