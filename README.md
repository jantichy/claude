...
**Volitelný krok mimo osu** – nedělá se pokaždé, ale u většího zadání se vyplatí. Nezávislý posudek dokumentu, který jsem psal s Claudem – pozicování, cenotvorby, datového modelu. Pošle na něj agenty, kteří **nemají z naší session žádný kontext** a čtou jenom soubory; každý s jiným úhlem: vnitřní rozpory, nevyslovené předpoklady, pre-mortem, pohled cílové skupiny, ekonomika. Každý nález pak dostane **nezávislého ověřovatele, který se ho snaží vyvrátit** – a ten na rozdíl od oponentů projektový kontext má, takže námitku stojící na tom, co oponent nemohl vědět, zabije stroj, ne já. Zamítnuté námitky se zapisují, aby je příští oponentura nenašla znovu jako nové.
