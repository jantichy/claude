...
- **Část znalostí v repu není.** `/compose` čte `~/Dev/context/compose/` a `~/Dev/context/archive/`, doménové znalosti žijí v `~/Dev/context/`, kde má každá doména vlastní adresář (`web/`, `admin/`, `coding/`, `text/`, `worktree/`, `analytics/`) – odkazuje na ně `CLAUDE.md` a většinu z nich používá skill `/standards`, který bez nich neudělá nic. To je moje soukromé know-how a osobní archiv, takže je tu nenajdete – ty skilly jsou k mání jako kostra, ne jako hotová věc.
