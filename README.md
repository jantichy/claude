...
## [`STRUCTURE.md`](STRUCTURE.md) – každý projekt vypadá stejně

Zjistil jsem, že v každém projektu chci ty samé čtyři věci vedle `CLAUDE.md`: `README.md` pro člověka a v `docs/` trojici `todo.md` (co jsem odložil), `decisions.md` (co jsem rozhodl a **proč**, včetně variant, které jsem zavrhl) a `rules.md` (principy, ve kterých se ten konkrétní projekt pohybuje). Nejcennější je na tom to poslední: jak projekt roste, vybrušují se hranice, ve kterých se rozhodujeme – a zapsané mi šetří to, abych je vymýšlel pokaždé znovu. Soubor tuhle konvenci definuje na jednom místě, `/project` ji instaluje a `/cleanup` kontroluje, že ji Claude opravdu průběžně plní.
