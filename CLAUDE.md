...
- `@~/.claude/GLOSSARY.md` – termíny, na kterých jsme se výslovně dohodli: co znamenají, v jakém rozsahu platí a co se jimi naopak neoznačuje
- `~/.claude/WORKTREE.md` – worktree layout projektu: kontejner s `.bare`, jeden pracovní adresář na větev, zakládání a dokončení větve. **Odkaz, ne import** – platí jen v projektu s tímhle uspořádáním, kde si ho natáhne stub v kořeni kontejneru; zapíná a ruší ho `/worktree`
